...
/**
 * Performance test for the MaterialMethod entity.
 */
public class MaterialMethodGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all materialMethods").get("/api/material-methods").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new materialMethod")
                        .post("/api/material-methods")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"type\": \"BUY\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_materialMethod_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created materialMethod").get("${new_materialMethod_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created materialMethod").delete("${new_materialMethod_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MaterialMethod entity").exec(scn);
